...
exports.listDraft = function(req, res) {
    Process.find({$and : [{ "kind": ['draft'] },{'company':req.user.company},{$or:[{'user': req.user.id},{'userProcess._id': req.user.id}] } ] }, function (err, drafts) {
        if (err) {
            return res.status(400).send({
                message: errorHandler.getErrorMessage(err)
            });
...
exports.listWaitingPublish = function(req, res) {
    Process.find({$and : [{ "kind": ['waitingPublish'] },{'company':req.user.company},{$or:[{'user': req.user.id},{'userProcess._id': req.user.id}] } ] }, function (err, waitings) {
        if (err) {
            return res.status(400).send({
                message: errorHandler.getErrorMessage(err)
            });
...
exports.listPublish = function(req, res) {
    Process.find({$and:[{ "kind": ['publish'] },{'company':req.user.company}]}, function (err, publish) {
        if (err) {
            return res.status(400).send({
                message: errorHandler.getErrorMessage(err)
            });
